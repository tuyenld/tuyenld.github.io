...
module Jekyll
  ...
  class GoogleAnalytics < Generator
    ...
    def generate(site)    
      ...
      if ga["jsonAuthentificationFile"]
        scope = 'https://www.googleapis.com/auth/analytics.readonly'
        auth = Google::Auth::ServiceAccountCredentials.make_creds(
             json_key_io: File.open(ga["jsonAuthentificationFile"]),
             scope: scope)
        credentials_access_token = auth.fetch_access_token!
      else
        ...
        if ga["debug"]
          Jekyll.logger.info "GA-debug (access_token): ", credentials_access_token['access_token']
        end
